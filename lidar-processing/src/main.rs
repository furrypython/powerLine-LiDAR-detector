...
fn execute_algorithms(input: &String, output: &String, all_files: &Vec<(String, las::Bounds)>, buffer_zone: f64) {
    ...
    // Macro-Plane Detection (RANSAC) to remove buildings/walls
    let now = time::Instant::now();
    let no_planes = semantics::remove_macro_planes(&elevated_points, 0.2, 2000, 100);
    println!("RANSAC Plane Detection time: {:?} millisecs. Extracted {} points.", now.elapsed().as_millis(), no_planes.len());

    // Volumetric Vegetation Filter to remove trees
    let now = time::Instant::now();
    let no_vegetation = semantics::remove_vegetation(&no_planes, 1.0, 0.2); // radius=1.0m, scattering_threshold=0.2
    println!("Vegetation Filter time: {:?} millisecs. Extracted {} points.", now.elapsed().as_millis(), no_vegetation.len());

    // Ensemble Filtering (PCA + Free Space + Droppers + Junctions + Collinearity)
    let now = time::Instant::now();
    let final_points = pca::extract_powerlines(&no_vegetation, 0.3, 0.85); // radius=0.3m, threshold=0.85
